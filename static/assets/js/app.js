...
(function () {
    ...
    fetch(`${apiUrl}/posts?page=${page}&per_page=10`, {
      ...
      .catch((error) => {
        ...
        postsContainer.innerHTML =
          '<div class="text-center text-muted p-3">暂无内容</div>';
        // 不需要隐藏顶部加载指示器
      });
  }
  ...
  function displayPosts(data) {
    ...
    if (!data.posts || data.posts.length === 0) {
      postsContainer.innerHTML =
        '<div class="text-center text-muted p-2">暂无内容</div>';
      return;
    }
    ...
    try {
      ...
      postsContainer.innerHTML =
        '<div class="alert alert-danger">显示帖子时出错</div>';
    }
  }
  ...
  function loadComments(postId, commentSection) {
    ...
    commentsList.innerHTML =
      '<div class="text-center p-3"><div class="spinner"></div><p>加载评论中...</p></div>';
  ...
  function updatePagination(current, total) {
    ...
    try {
      ...
      html += `<li class="page-jump">
        <input min="1" max="${total}" class="page-input" style="width: 30px; text-align: center;">
        ...
        if (postElement) {
          ...
          if (commentBtn) {
            ...
            commentBtn.className =
              "btn btn-sm btn-outline-primary show-comments";
          }
        }
